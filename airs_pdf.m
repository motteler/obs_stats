...
p1 = polyfit(ynum2, tint, 1)
tfit = polyval(p1, ynum2);

figure(1); clf
pcolor(ynum2, tmid, tdif3)
title(sprintf('%d-day PDF difference from mean', k2))
ylabel('Tb bins, K')
xlabel('year')
% caxis([-3e-3, 3e-3])
% caxis([-2e-3, 2e-3])
...
title(sprintf('%d-day PDF integrated Tb', k2))
